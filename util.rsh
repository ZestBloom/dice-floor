...
export const requireTok8 = (A) => {
  ...
  A.publish(tok0, tok1, tok2, tok3, tok4, tok5, tok6, tok7);
  require(tok0 != tok1);
  require(tok0 != tok2);
  require(tok0 != tok3);
  require(tok0 != tok4);
  ...
  require(tok0 != tok7);
  commit();
  return { tokens: [tok0, tok1, tok2, tok3, tok4, tok5, tok6, tok7] };
};
...
export const requireTok5WithFloorRewardDeadline = (A, addr) => {
  A.only(() => {
    const {
      price,
      reward,
  ...
  A.publish(tok0, tok1, tok2, tok3, tok4, price, reward).timeout(
    relativeTime(10), // XXX
    () => {
      Anybody.publish();
      commit();
  ...
  require(A == addr);
  commit();
  return { tokens: [tok0, tok1, tok2, tok3, tok4], price, reward };
};
